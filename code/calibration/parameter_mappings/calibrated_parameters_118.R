...
VERSION.MANAGER = register.parameters.prior(VERSION.MANAGER,
                                            prior = join.distributions(BASE.PARAMETERS.PRIOR, EHE.PARAMETERS.PRIOR),
                                            version = 'collapsed_1.0')

BASE.PARAMETER.VAR.BLOCKS = list(
...
VERSION.MANAGER = register.parameter.sampling.blocks(VERSION.MANAGER,
                                                     blocks = c(BASE.PARAMETER.VAR.BLOCKS, EHE.PARAMETER.VAR.BLOCKS),
                                                     version = 'collapsed_1.0')
...
##-- UPDATES FOR PROJECTIONS --##

CURRENT.GAINS.END.BY.YEAR = Uniform.Distribution(2020,2025)
TOTAL.FUTURE.SLOPE.OR.DIST = Lognormal.Distribution(0, log(1.05))

BASE.PROJECTION.PARAMETERS.DISTRIBUTION = join.distributions(
    testing.gains.end.by.year = CURRENT.GAINS.END.BY.YEAR,
    prep.gains.end.by.year = CURRENT.GAINS.END.BY.YEAR,
    
    total.future.testing.slope.or = TOTAL.FUTURE.SLOPE.OR.DIST,
    total.future.prep.slope.or = TOTAL.FUTURE.SLOPE.OR.DIST,
    
    future.slope.after.year = Constant.Distribution(2020)
)

EHE.PROJECTION.PARAMTERS.DISTRIBUTION = join.distributions(
    suppression.gains.end.by.year = CURRENT.GAINS.END.BY.YEAR,
    total.future.suppressed.slope.or = TOTAL.FUTURE.SLOPE.OR.DIST
)
...
VERSION.MANAGER = register.projection.parameters.distribution(
    ...
    distribution = join.distributions(BASE.PROJECTION.PARAMETERS.DISTRIBUTION,
                                      EHE.PROJECTION.PARAMTERS.DISTRIBUTION),
    version = 'collapsed_1.0'
)

BASE.UPDATE.COMPONENTS.FUNCTION <- function(parameters, components,
                                            data.managers=ALL.DATA.MANAGERS)
{
...
EHE.UPDATE.COMPONENTS.FUNCTION <- function(parameters, components,
                                           data.managers=ALL.DATA.MANAGERS)
{
    components = set.background.change.to.years(components,

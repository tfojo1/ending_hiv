...
if (1==2)
{
    ...
    save(ALL.DATA.MANAGERS, file='../code/cached/ALL.DATA.MANAGERS.Rdata')
}
...
{
    intersect(
        as.numeric(dimnames(cm$testing$total.mean.log.odds)[[1]]),
...
#                         orig.code=ever.df$Locationabbr,
    ...
    df = NULL
    df.paired = data.frame(msa=dimnames(cm$testing$total.mean.log.odds)[['msa']])
    for (year in dimnames(cm$testing$total.mean.log.odds)[['year']])
    {
        ...
        df = rbind(df,
                   data.frame(year=year,
                              msa=dimnames(cm$testing$total.mean.log.odds)[['msa']],
                              log.odds.het = cm$testing$total.mean.log.odds[year,,'heterosexual'],
                              log.odds.msm = cm$testing$total.mean.log.odds[year,,'msm'],
                              log.odds.idu = cm$testing$total.mean.log.odds[year,,'idu'],
                              log.odds.ever.tested = ever.tested[dimnames(cm$testing$total.mean.log.odds)[['msa']]]))

        one.df = data.frame(log.odds.het = cm$testing$total.mean.log.odds[year,,'heterosexual'],
                            log.odds.msm = cm$testing$total.mean.log.odds[year,,'msm'],
                            log.odds.idu = cm$testing$total.mean.log.odds[year,,'idu'])
#                            log.odds.ever.tested = ever.tested[dimnames(cm$testing$total.mean.log.odds)[['msa']]])
    ...
    cm$testing$total.mean.log.odds['2015', missing.het.codes, 'heterosexual'] = preds$fit
    cm$testing$total.se.log.odds['2015', missing.het.codes, 'heterosexual'] = preds$se.fit
                                           ...
                                           year)
{
    ...
    if (any(dimnames(cm.subset$total.mean.log.odds)[[1]]==year) &&
        any(dimnames(cm.subset$total.mean.log.odds)[[2]]==msa.code) &&
        !is.na(cm.subset$total.mean.log.odds[year, msa.code,]))
    {
        list(mean=cm.subset$total.mean.log.odds[year, msa.code,],
                                                            ...
                                                            months)
{
    ...
    {
        one.log.mean = c(total=log.means[i])
        one.log.se = c(total=log.ses[i])
                                              ...
                                              backup.months = ifelse(months==3, 1, 3))
{
    ...
    if (any(dimnames(cm.subset[[mean.name]])[[1]]==year) &&
        ...
        !is.na(cm.subset[[mean.name]][year, msa.code,]))
    {
        rv = list(mean=cm.subset[[mean.name]][year, msa.code,],
                se=cm.subset[[mean.name]][year, msa.code,])
    }
                                              ...
                                              months)
{
    do.set.continuum.distributions(cm.subset,
                                   ...
                                   value.names = c('black','hispanic','age1','age2','age4','age5','msm','female','idu'),
                                   log.means = log.means,
                                   log.ses = log.ses,
                                   msa.code = msa.code,
                                   year = year,

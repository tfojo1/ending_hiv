...
test.run.simulation <- function(...)
{
    ...
    wrong.name.mask = !sapply(names(pp), function(param.name){
        any(names(parameters)==param.name)
    })
    if (any(wrong.name.mask))
        stop(paste0("The following parameters are not correctly named: ",
                    paste0("'", names(pp)[wrong.name.mask], "'", collapse=', '))
             )

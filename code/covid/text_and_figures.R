...
##-- Narrative absent COVID --##
if (1==2)
{
    abs.inc.diff.2025.2019 = outcomes.arr[,'baseline',,'2025','incidence',] - 
        outcomes.arr[,'baseline',,'2019','incidence',]
    rel.inc.diff.2025.2019 = abs.inc.diff.2025.2019 / outcomes.arr[,'baseline',,'2019','incidence',] 
    range(apply(rel.inc.diff.2025.2019, 'location', mean))
    
    abs.agg.inc.diff.2025.2019 = apply(abs.inc.diff.2025.2019, 'sim', sum)
    rel.agg.inc.diff.2025.2019 = apply(abs.inc.diff.2025.2019, 'sim', sum) / 
        apply(outcomes.arr[,'baseline',,'2019','incidence',], 'sim', sum)
}

##-- Narrative for diagnoses --##
...
if (1==2)
{
    ...
    qplot(rel.diff.from.prior[,,'2020','new',])
    qplot(rel.diff.from.prior[,,'2020','incidence',])
    qplot(rel.diff.from.prior[,,'2020','new',], rel.diff.from.prior[,,'2020','incidence',])
    
    
    
    apply(abs.diff.from.prior[,,,'new',]<0, 'year', mean)
    apply(abs.diff.from.prior[,,,'new',]<0, c('scenario','year'), mean)
    apply(abs.diff.from.prior[,,,'incidence',]<0, 'year', mean)
    apply(abs.diff.from.prior[,,,'incidence',]<0, c('scenario','year'), mean)
    
    
    
    abs.diff.new.from.prior = outcomes.arr[,ALL.COVID.SCENARIOS,,as.character(YEARS.OF.INTEREST[-1]),'new',] - 
        outcomes.arr[,ALL.COVID.SCENARIOS,,as.character(YEARS.OF.INTEREST[-length(YEARS.OF.INTEREST)]),'new',]
    rel.diff.new.from.prior = abs.diff.new.from.prior /  
        outcomes.arr[,ALL.COVID.SCENARIOS,,as.character(YEARS.OF.INTEREST[-length(YEARS.OF.INTEREST)]),'new',]
    apply(abs.diff.new.from.prior<0, 'year', mean)
    apply(abs.diff.new.from.prior<0, c('scenario','year'), mean)
    round(100*apply(rel.diff.new.from.prior, 'year', function(x){c(mean(x), quantile(x, probs=c(.025, .975)))}))
    
    
    
    abs.diff.new.from.2019 = apply(outcomes.arr[,ALL.COVID.SCENARIOS,,as.character(YEARS.OF.INTEREST[-1]),'new',], 
                                   'year',
                                   function(x){
                                      x - outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','new',]
                                   })
    dim.names = dimnames(abs.diff.new.from.prior)[c(setdiff(names(dim(abs.diff.new.from.prior)), 'year'), 'year')]
    dim(abs.diff.new.from.2019) = sapply(dim.names, length)
    dimnames(abs.diff.new.from.2019) = dim.names
    
    
    abs.diff.new.2020.2019 = outcomes.arr[,ALL.COVID.SCENARIOS,,'2020','new',] - outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','new',]
    rel.diff.new.2020.2019 = abs.diff.new.2020.2019 / outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','new',]
    mean(abs.diff.new.2020.2019<0)
    apply(abs.diff.new.2020.2019<0, 'scenario', mean)
    c(mean(abs.diff.new.2020.2019), quantile(abs.diff.new.2020.2019, probs=c(0.025, 0.975)))
    round(100*c(mean(rel.diff.new.2020.2019), quantile(rel.diff.new.2020.2019, probs=c(0.025, 0.975))))
    
    abs.diff.new.2021.2019 = outcomes.arr[,ALL.COVID.SCENARIOS,,'2021','new',] - outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','new',]
    rel.diff.new.2021.2019 = abs.diff.new.2021.2019 / outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','new',]
    mean(abs.diff.new.2021.2019<0)
    apply(abs.diff.new.2021.2019<0, 'scenario', mean)
    round(100*c(mean(rel.diff.new.2021.2019), quantile(rel.diff.new.2021.2019, probs=c(0.025, 0.975))))
    
    abs.diff.new.2021.2020 = outcomes.arr[,ALL.COVID.SCENARIOS,,'2021','new',] - outcomes.arr[,ALL.COVID.SCENARIOS,,'2020','new',]
    rel.diff.new.2021.2020 = abs.diff.new.2021.2020 / outcomes.arr[,ALL.COVID.SCENARIOS,,'2020','new',]
    mean(abs.diff.new.2021.2020<0)
    apply(abs.diff.new.2021.2020<0, 'scenario', mean)   
    round(100*c(mean(rel.diff.new.2021.2020), quantile(rel.diff.new.2021.2020, probs=c(0.025, 0.975))))
    
    abs.diff.new.2022.2021 = outcomes.arr[,ALL.COVID.SCENARIOS,,'2022','new',] - outcomes.arr[,ALL.COVID.SCENARIOS,,'2021','new',]
    rel.diff.new.2022.2021 = abs.diff.new.2022.2021 / outcomes.arr[,ALL.COVID.SCENARIOS,,'2021','new',]
    mean(abs.diff.new.2022.2021<0)
    apply(abs.diff.new.2022.2021<0, 'scenario', mean)   
    round(100*c(mean(rel.diff.new.2022.2021), quantile(rel.diff.new.2022.2021, probs=c(0.025, 0.975))))
    ...
    # Inc
    abs.diff.inc.2020.2019 = outcomes.arr[,ALL.COVID.SCENARIOS,,'2020','incidence',] - outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','incidence',]
    rel.diff.inc.2020.2019 = abs.diff.inc.2020.2019 / outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','incidence',]
    mean(abs.diff.inc.2020.2019<0)
    apply(abs.diff.inc.2020.2019<0, 'scenario', mean)
    c(mean(abs.diff.inc.2020.2019), quantile(abs.diff.inc.2020.2019, probs=c(0.025, 0.975)))
    round(100*c(mean(rel.diff.inc.2020.2019), quantile(rel.diff.inc.2020.2019, probs=c(0.025, 0.975))))
    
    
    abs.diff.inc.2021.2019 = outcomes.arr[,ALL.COVID.SCENARIOS,,'2021','incidence',] - outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','incidence',]
    rel.diff.inc.2021.2019 = abs.diff.inc.2021.2019 / outcomes.arr[,ALL.COVID.SCENARIOS,,'2019','incidence',]
    mean(abs.diff.inc.2021.2019<0)
    apply(abs.diff.inc.2021.2019<0, 'scenario', mean)
    round(100*c(mean(rel.diff.inc.2021.2019), quantile(rel.diff.inc.2021.2019, probs=c(0.025, 0.975))))
    
    abs.diff.inc.2021.2020 = outcomes.arr[,ALL.COVID.SCENARIOS,,'2021','incidence',] - outcomes.arr[,ALL.COVID.SCENARIOS,,'2020','incidence',]
    rel.diff.inc.2021.2020 = abs.diff.inc.2021.2020 / outcomes.arr[,ALL.COVID.SCENARIOS,,'2020','incidence',]
    mean(abs.diff.inc.2021.2020<0)
    apply(abs.diff.inc.2021.2020<0, 'scenario', mean)   
    round(100*c(mean(rel.diff.inc.2021.2020), quantile(rel.diff.inc.2021.2020, probs=c(0.025, 0.975))))
...
if (1==2)
{
    cuml = apply(outcomes.arr[,,1,as.character(YEARS.OF.INTEREST),,], setdiff(names(dim(outcomes.arr)), c('year','intervention')), sum)
    abs.cuml.diff = cuml[,ALL.COVID.SCENARIOS,,]; for(sc in ALL.COVID.SCENARIOS) { 
        abs.cuml.diff[,sc,,] = cuml[,sc,,] - cuml[,'baseline',,]
    } 
    rel.cuml.diff = abs.cuml.diff; for(sc in ALL.COVID.SCENARIOS) { 
        rel.cuml.diff[,sc,,] = abs.cuml.diff[,sc,,] / cuml[,'baseline',,]
    }
    
    agg.cuml = apply(cuml, setdiff(names(dim(cuml)), 'location'), sum)
    abs.agg.cuml.diff = apply(abs.cuml.diff, setdiff(names(dim(agg.cuml)), 'location'), sum)
    rel.agg.cuml.diff = abs.agg.cuml.diff; for(sc in ALL.COVID.SCENARIOS) { 
        rel.agg.cuml.diff[sc,,] = rel.agg.cuml.diff[sc,,] / agg.cuml['baseline',,]
    }
    
    
    # The difference in cumulative incidence (city-level)
    round(100*cbind(mean=apply(rel.cuml.diff[,,'incidence',], 'scenario', mean),
                    ci.lower=apply(rel.cuml.diff[,,'incidence',], 'scenario', quantile, probs=0.025),
                    ci.upper=apply(rel.cuml.diff[,,'incidence',], 'scenario', quantile, probs=0.975)
                    ))
    
    # The difference in aggregate incidence
    round(100*cbind(mean=apply(rel.agg.cuml.diff[,'incidence',], 'scenario', mean),
                    ci.lower=apply(rel.agg.cuml.diff[,'incidence',], 'scenario', quantile, probs=0.025),
                    ci.upper=apply(rel.agg.cuml.diff[,'incidence',], 'scenario', quantile, probs=0.975)
    ))
    
    format(round(cbind(mean=apply(abs.agg.cuml.diff[,'incidence',], 'scenario', mean),
                    ci.lower=apply(abs.agg.cuml.diff[,'incidence',], 'scenario', quantile, probs=0.025),
                    ci.upper=apply(abs.agg.cuml.diff[,'incidence',], 'scenario', quantile, probs=0.975)
    )), big.mark = ',')
    
    
    cum.inc = cum[,,'incidence',]
    rel.cum.inc = rel.cum[,,'incidence',]
    
    t(apply(rel.cum.inc, 'scenario', range))

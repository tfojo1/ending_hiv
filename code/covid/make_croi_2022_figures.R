...
##-- BOXPLOT --##
LOCATION.BOXPLOT.PANEL.HEIGHT = 3
...
if (1==2)
{
    ...
    make.location.boxplot(colors=COLORS[3],
                          ...
                          include.total = F,
                          outcome.axis.name = 'Change in Cumulative HIV Incidence\n2020-25, Relative to No-COVID',
                          vertical = F) + 
        BOXPLOT.THEME +
        xlab(NULL) 
    dev.off()
...
SCATTER.THEME = theme(text = element_text(size=8),
                      axis.title = element_text(size=7),
                      legend.position = 'none')
SCATTER.PANEL.HEIGHT = 2
SCATTER.PANEL.WIDTH = 3
...
if (1==2)
{
    ...
    make.covid.scatterplot(results = big.supp.drop.results,
        ...
        ylab("Median Change in Cumulative HIV Incidence\n2020-25, Relative to No-COVID") +

...
#'@param facet.by The names of dimensions according to which to 'facet' (a separate panel for each) - this should be a subset of names(get.facet.by.options(location))
#'@param split.by The names of dimensions according which to split the plot (a separate line on each panel) - this should be a subset of names(get.split.by.options(location))
#'@param dimension.subsets A named list of which values for each dimension to include in the plot. The names of the list are the named dimensions as given by names(get.dimension.value.options()), and the values for dimension d must be a subset of get.dimension.values()[[d]]
#'@param plot.format The character string indicating the format for the plot - this should be one of names(get.plot.format.options(location))
#'@param show.truth Whether to include "truth" (known from epi surveillance) in the plot
#'
                             ...
                             truth.point.size=5)
{
    ...
    for (d in 1:length(data.types))
    {
        ...
        {
            base.df = rbind(base.df,
                       suppressWarnings(data.frame(data_type=data.types[d],
    ...
    facet.formula = as.formula(paste0('~data_type + ', paste0(facet.by, collapse='+')))
    
    plot = ggplot(df, aes(year, value, color=intervention, group=split.by)) + 
        geom_line() +
...
##------------------------------------------------##
##-- FUNCTIONS TO GET THE OPTIONS FOR ARGUMENTS --##
##------------------------------------------------##
...
##---------------------------------------------##
##-- FUNCTIONS TO DEFINE WHICH OPTIONS APPLY --##
##---------------------------------------------##
...
DIMENSION.VALUES = list(age=AGES,
                        race=RACES,
                        sex=SEXES,
                        risk=RISKS)

DIMENSIONS = c(age='Age',
           race='Race',
           sex='Sex',
           risk='HIV Risk Factor')

DATA.TYPES = c(new='Reported Diagnoses',
